...
nomFichier=[fileName '_MEA_MOVING-Circle_' num2str(radius_um) 'um_' int2str(c(3)) '.' int2str(c(2)) '.' int2str(c(1)) '_' int2str(c(4)) 'h' int2str(c(5)) '.bin'];
...
%     DMD2STIM = 255*(hypot(x-DMD.coord.Left(2), y-DMD.coord.Left(1))< distanceBetweenElectrodes/2);
    ...
    innerCircle = (hypot(x-center(2), y-center(1))< radius_pix-thickness/2);
	outerCircle = (hypot(x-center(2), y-center(1))< radius_pix+thickness/2);
