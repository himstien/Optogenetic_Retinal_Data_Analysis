...
%     gratingSize_um = gratingSize*(distanceBetweenElectrodes/100); %um
    ...
    for radius = 1:phaseMax %:phaseMax
        ...
        outerRadius = sqrt(radius*radius + (constArea/(pi)));
        gratingSize = outerRadius - radius;
...
nomFichier=[filePath '/' 'Circular_Bar_DMD_' num2str(DMD) '_constArea_centerd_Fovea_' num2str(screenWidth) '_' num2str(screenHeight) '_normal_' date '.bin'];
...
save([filePath '/' 'Circular_Bar_Stimulation_constArea_binFile_Reference_normal_' date '.mat'], 'frameReference', 'center', 'center_TIFF',  '-v7.3');
...
nomFichier=[filePath '/' 'circular_Bar_constArea_' num2str(nbReps) '_timeOn_' num2str(periode) '_' date '.vec'];
...
nomFichier=[filePath '/' 'circular_Bar_constArea_' num2str(nbReps) '_timeOn_' num2str(periode) '_' date '_reverse.vec'];
