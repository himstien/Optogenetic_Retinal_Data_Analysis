...
useFoveaCenter = 1;
if (useFoveaCenter)
    defaultPath = ['/home/icub/POST_DOC/DATA/' date '_primate'];
    filePath = [defaultPath '/'];
    test = input('Enter test number for calibration figures: ', 's');
    fileName = 'test';

    DMD_filename = [filePath fileName test 'c.tiff'];
    [X_DMD, Y_DMD, DMD_Image] = calibrationRepere_HA(DMD_filename);
%     [X_DMD, Y_DMD, DMD_Image] = calibrationRepere(DMD_filename);

%     floresence_fileName = [filePath fileName test 'f.tif'];
    floresence_fileName = [filePath fileName test 'p.tiff'];    

    center = getFoveaCenter(floresence_fileName, X_DMD, Y_DMD, DMD);
else
...
nomFichier=['Circular_Bar_DMD_' num2str(DMD) '_centerd_Fovea_' num2str(screenWidth) '_' num2str(screenHeight) '_normal_' date '.bin'];
...
save(['Circular_Bar_Stimulation_binFile_Reference_normal_' date '.mat'], 'frameReference', '-v7.3');
...
nomFichier=['circular_Bar_' num2str(nbReps) '_timeOn_' num2str(periode) '_' date '.vec'];
...
nomFichier=['circular_Bar_' num2str(nbReps) '_timeOn_' num2str(periode) '_' date '_reverse.vec'];
